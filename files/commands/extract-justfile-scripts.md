Extract embedded shell code from justfile recipes into separate script files, then generate test coverage.
...
3. **For each embedded script:**
   - Extract the shell code to `scripts/<namespace>/<recipe-name>.sh` (use the justfile module name as namespace, or `iso`/`usb`/etc. based on existing directory conventions)
...
5. **Chain to coverage sub-skill** — run `/cover-sh` to generate bats tests for the newly extracted scripts
...
- The coverage commit from `/cover-sh` should be a separate follow-up commit
